...
nconf.argv({
  ...
  'site': {
    describe: 'Site: Name of site_id.'
  },
  'target': {
    describe: 'Target: Device to run test.'
  },
  'environment': {
    describe: 'Target: Environment to run test.'
  }
}).env();
...
async.waterfall([
  ...
  if(opts.target)
    //fs.rmdir(path.join(opts.reportsDirectory, opts.target));
